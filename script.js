...
/**
 * Spielt einen悦耳igen Sound für richtige Antwort
 */
...
function renderManage(filter = '') {
    ...
    if (emptyState) emptyState.classList.toggle('hidden', (subs.length > 0 || cards.length > 0) || curFolder !== null);
...
function updateLearnSource() {
    ...
    // Standard: Fällige Karten vorauswählen
    s.value = 'due';
}
...
function startSession() {
    ...
    if (src === 'due') {
        ...
        if (pool.length === 0) {
            showNotification('Keine fälligen Karten! Lerne einfach weiter oder erweitere den Lernumfäng.', 'info');
            return;
        }
    } else if (src === 'all') {
        pool = [...data.cards];
